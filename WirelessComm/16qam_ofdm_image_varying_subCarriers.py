...
# ==========================
# Evaluation Under Varying Subcarriers
# ==========================
...
subcarrier_options = [32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256]  # Different subcarrier configurations
ber_sub_list = []
ser_sub_list = []
papr_sub_list = []

for sc in subcarrier_options:
    subCarriers = sc
    ...
    for i in range(0, len(image_bits), totalData_per_symbol):
        ...
        bits_SP = bits_chunk.reshape((len(dataCarriers), mu))
        qam_VAL = encoder(bits_SP)
        data_OFDM = create_symbol(qam_VAL)
        time_OFDM = ifft(data_OFDM)
        cp_OFDM = add_cp(time_OFDM)
    ...
    ser_sub_list.append(ser)
    papr_sub_list.append(avg_papr)

# Plotting Evaluation for Subcarriers
plt.figure()
plt.plot(subcarrier_options, ber_sub_list, marker='o')
plt.title("BER vs Subcarriers")
plt.xlabel("Number of Subcarriers")
plt.ylabel("Bit Error Rate")
plt.grid(True)
plt.savefig("16qam_ber_vs_sc.png")

plt.figure()
plt.plot(subcarrier_options, ser_sub_list, marker='x', color='orange')
plt.title("SER vs Subcarriers")
plt.xlabel("Number of Subcarriers")
plt.ylabel("Symbol Error Rate")
plt.grid(True)
plt.savefig("16qam_ser_vs_sc.png")

plt.figure()
plt.plot(subcarrier_options, papr_sub_list, marker='s', color='green')
plt.title("PAPR vs Subcarriers")
plt.xlabel("Number of Subcarriers")
plt.ylabel("Peak to Average Power Ratio (dB)")
plt.grid(True)
plt.savefig("16qam_papr_vs_sc.png")