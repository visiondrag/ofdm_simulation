...
mu = 8
totalData_per_symbol = len(dataCarriers) * mu  # now 8 bits per QAM symbol
# 256-QAM Mapping Table: Gray-coded constellation for 8 bits per symbol
def generate_256qam_mapping():
    mapping = {}
    real_vals = np.arange(-15, 16, 2)
    imag_vals = np.arange(-15, 16, 2)
    bits = [list(np.binary_repr(i, width=8)) for i in range(256)]
    bits = [tuple(int(b) for b in bit) for bit in bits]
    for idx, bit in enumerate(bits):
        real = real_vals[idx % 16]
        imag = imag_vals[idx // 16]
        mapping[bit] = complex(real, imag)
    return mapping

mapping_table = generate_256qam_mapping()

# ==> each OFDM symbol is subCarriers - pilot = 64 - 9 = 55 usable for data (dataCarriers)
# ==> each OFDM symbol is 55 * 8 = 440 bits/symbol

plotCarriers(   
    pilotCarriers=pilotCarriers,
    dataCarriers=dataCarriers,
    subCarriers=subCarriers,
    name="256qam_carriers.png"
)

plotConstellationMap(
    mapping_table=mapping_table,
    mu=mu,
    name="256qam_constellation_map.png"
)
...
plotChannelResponse(
    ...
    subCarriers=subCarriers,
    name="256qam_channel_response.png"
)
...
bits = np.random.binomial(n=1, p=0.5, size=totalData_per_symbol) # generate binary data using Bernoulli distribution
                                                                 # total num of data bits (excluding pilots) = 440
def s_to_p(bits):
...
plotChannelEstimation(
        ...
        name="256qam_channel_estimation.png"
    )
...
plotRecvConstellation(
    ...
    mu=mu, 
    name="256qam_received_constellation.png"
)
...
plotDecodingPoints(
    ...
    mu=mu, 
    name="256qam_constellation_decoding.png"
)
