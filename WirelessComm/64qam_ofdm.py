...
mu = 6
totalData_per_symbol = len(dataCarriers) * mu  # now 6 bits per QAM symbol
mapping_table = {
...
# ==> each OFDM symbol is 55 * 6 = 330 bits/symbol

plotCarriers(   
    pilotCarriers=pilotCarriers,
    dataCarriers=dataCarriers,
    subCarriers=subCarriers,
    name="64qam_carriers.png"
)

plotConstellationMap(
    mapping_table=mapping_table,
    mu=mu,
    name="64qam_constellation_map.png"
)
...
plotChannelResponse(
    ...
    subCarriers=subCarriers,
    name="64qam_channel_response.png"
)
...
bits = np.random.binomial(n=1, p=0.5, size=totalData_per_symbol) # generate binary data using Bernoulli distribution
                                                                 # total num of data bits (excluding pilots) = 330
def s_to_p(bits):
...
plotChannelEstimation(
        ...
        name="64qam_channel_estimation.png"
    )
...
plotRecvConstellation(
    ...
    mu=mu, 
    name="64qam_received_constellation.png"
)
...
plotDecodingPoints(
    ...
    mu=mu, 
    name="64qam_constellation_decoding.png"
)
